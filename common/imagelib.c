...
void LoadImageSTB(const char *name, byte **pixels, int *width, int *height);
  ...
  *pic = out;
  ...
  for (y = 0; y <= pcx->ymax; y++, pix += pcx->xmax + 1) {
    for (x = 0; x <= pcx->xmax;) {
      ...
      // FIXME: this shouldn't happen, but it does.  Are we decoding the file
      // wrong? Truncate runLength so we don't overrun the end of the buffer
...
==============
*/
void LoadBMP(const char *filename, byte **pic, byte **palette, int *width,
             int *height) {
  byte *out;
  FILE *fin;
  int i;
  int bfOffBits;
  int structSize;
  int bcWidth = 0, bcHeight = 0, bcPlanes = 0, bcBitCount = 0;
  byte bcPalette[1024];
  qboolean flipped;
...
void Load256Image(const char *name, byte **pixels, byte **palette, int *width,
  ...
  } else if (!Q_stricmp(ext, "png") || !Q_stricmp(ext, "jpg") || !Q_stricmp(ext, "jpeg") || !Q_stricmp(ext, "tga")) {
    int x, y, n;
...
void LoadTGABuffer(byte *buffer, int buflen, byte **pic, int *width, int *height) {
  ...
  if (!data) {
    Error("Failed to load TGA from buffer: %s", stbi_failure_reason());
  }
  *width = x;
  *height = y;
  *pic = (byte *)data;
...
void Load32BitImage(const char *name, unsigned **pixels, int *width,
                    int *height) {
  LoadImageSTB(name, (byte **)pixels, width, height);
}
