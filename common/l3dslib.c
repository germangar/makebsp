...
// Alias stores triangles as 3 explicit vertices in .tri files, so even though
// we start out with a vertex pool and vertex indices for triangles, we have to
// convert to raw, explicit triangles
...
void Load3DSTriangleList(char *filename, triangle_t **pptri,
                         int *numtriangles) {
  FILE *input;
  short int tshort;
  ...
  *pptri = ptri;
  ...
  // parse through looking for the relevant chunk tree (MAIN3DS | EDIT3DS |
  // EDIT_OBJECT | OBJ_TRIMESH | {TRI_VERTEXL, TRI_FACEL1}) and skipping other
  // chunks
  ParseChunk(input);
