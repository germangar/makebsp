...
static void ByteSwapTri(tf_triangle *tri) {
  ...
  for (i = 0; i < sizeof(tf_triangle) / 4; i++) {
    int val;
    memcpy(&val, (char *)tri + i * 4, 4);
    val = BigLong(val);
    memcpy((char *)tri + i * 4, &val, 4);
  }
}

static void ReadPolysetGeometry(triangle_t *tripool, FILE *input, int count,
                                triangle_t *ptri) {
  tf_triangle tri;
  int i;
...
void TRI_LoadPolysets(const char *filename, polyset_t **ppPSET, int *numpsets) {
  ...
  *((unsigned char *)&exitpattern + 3) = *((unsigned char *)&t + 0);
  ...
  if (BigLong(magic) != MAGIC)
    Error("%s is not a Alias object separated triangle file, magic number is "
          "wrong.",
          filename);
  ...
  *ppPSET = pPSET;
  ...
  while (feof(input) == 0) {
    if (fread(&start, sizeof(float), 1, input) < 1)
      break;
    int start_int;
    memcpy(&start_int, &start, 4);
    start_int = BigLong(start_int);
    if (start_int != exitpattern) {
      if (start == FLOAT_START) {
        ...
        if (i != 0) {
          ...
          pPSET[pset].name[sizeof(pPSET[pset].name) - 1] = '\0';
        } else
          strcpy(pPSET[pset].name, "(unnamed)");
        strlwr(pPSET[pset].name);

        //				indent();
        //				fprintf(stdout,"OBJECT START:
        //%s\n",name);
        ...
        if (count != 0) {
          ...
          /*
                                                  if ( i != 0 )
                                                          strncpy(
             pPSET[pset].texname, tex, sizeof( pPSET[pset].texname ) - 1 ); else
                                                          strcpy(
             pPSET[pset].texname, "(unnamed)" ); strlwr( pPSET[pset].texname );
          */

          //					indent();
          //					fprintf(stdout,"  Object texture
          // name: '%s'\n",tex);
        }
