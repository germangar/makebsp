...
protected:
  ...
  StrPtr(const char *pStr, T *p) : Str(pStr) { m_pPtr = p; }
...
PACKDirPtr ReadPACKDirectory(FILE *packfile, UInt32 offset, UInt16 *dirsize_r) {
  ...
  *dirsize_r = 0;
  ...
  if ((fseek(packfile, offset, SEEK_SET) < 0) ||
      ...
      (ReadInt32(packfile, &size) == FALSE) || (size == 0L) ||
...
boolean AddToFileListAlphabetized(FILELIST **list, char *filename,
                                  UInt32 offset, UInt32 size, boolean dirs) {
...
boolean OpenPK3(const char *filename) {
  ...
  if (zFile != NULL) {
    ...
    while (nStatus == UNZ_OK) {
      cFilename[0] = '\0';
      unzGetCurrentFileInfo(*zFile, &zInfo, cFilename, WORK_LEN, NULL, 0, NULL,
                            0);
      strlwr(cFilename);
...
void closePK3(unzFile zf) { unzClose(zf); }
...
void OpenPakFile(const char *filename) {
  ...
  for (i = 0; i < dirsize; i++) {
    if (!strnicmp("textures/", pakdirptr[i].name, 9)) {
      ...
      AddToFileListAlphabetized(&(dummy->files), str1, pakdirptr[i].offset,
                                pakdirptr[i].size, true);
