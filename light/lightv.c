...
#define LIGHTMAP_SIZE (g_game->lightmapSize)
...
void VL_DrawLightmapPixel(int surfaceNum, int x, int y, int color) {
  ...
  if (ds->surfaceType == MST_PATCH) {
    ...
    VectorCopy(
        mesh->verts[(y - ds->lightmapOffset[0][1]) * mesh->width + x - ds->lightmapOffset[0][0]].xyz,
        w.points[0]);
    VectorCopy(
        mesh->verts[(y + 1 - ds->lightmapOffset[0][1]) * mesh->width + x - ds->lightmapOffset[0][0]]
            .xyz,
        w.points[1]);
    VectorCopy(mesh->verts[(y + 1 - ds->lightmapOffset[0][1]) * mesh->width + x + 1 -
                           ds->lightmapOffset[0][0]]
                   .xyz,
               w.points[2]);
    VectorCopy(
        mesh->verts[(y - ds->lightmapOffset[0][1]) * mesh->width + x + 1 - ds->lightmapOffset[0][0]]
            .xyz,
        w.points[3]);
    w.numpoints = 4;
  } else {
    VectorMA(ds->lightmapOrigin, (float)x - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][0],
             ds->lightmapVecs[0], w.points[0]);
    VectorMA(w.points[0], (float)y - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][1],
             ds->lightmapVecs[1], w.points[0]);
    VectorMA(ds->lightmapOrigin, (float)x - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][0],
             ds->lightmapVecs[0], w.points[1]);
    VectorMA(w.points[1], (float)y - LIGHTMAP_PIXELSHIFT + 1 - ds->lightmapOffset[0][1],
             ds->lightmapVecs[1], w.points[1]);
    VectorMA(ds->lightmapOrigin,
             (float)x - LIGHTMAP_PIXELSHIFT + 1 - ds->lightmapOffset[0][0],
             ds->lightmapVecs[0], w.points[2]);
    VectorMA(w.points[2], (float)y - LIGHTMAP_PIXELSHIFT + 1 - ds->lightmapOffset[0][1],
             ds->lightmapVecs[1], w.points[2]);
    VectorMA(ds->lightmapOrigin,
             (float)x - LIGHTMAP_PIXELSHIFT + 1 - ds->lightmapOffset[0][0],
             ds->lightmapVecs[0], w.points[3]);
    VectorMA(w.points[3], (float)y - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][1],
             ds->lightmapVecs[1], w.points[3]);
    w.numpoints = 4;
  }
...
=============
*/
int VL_SplitWinding(winding_t *in, winding_t *back, plane_t *split,
                    float epsilon) {
  vec_t dists[128];
  int sides[128];
  int counts[3];
  ...
  for (i = 0; i < in->numpoints; i++) {
    ...
    if (neww->numpoints >= MAX_POINTS_ON_FIXED_WINDING) {
      _printf("WARNING: VL_SplitWinding -> MAX_POINTS_ON_FIXED_WINDING "
              "overflowed\n");
...
=====================
*/
void VL_TextureMatrixFromPoints(lFacet_t *f, drawVert_t *a, drawVert_t *b,
                                drawVert_t *c) {
  int i, j;
  float t;
  float m[3][4];
...
=====================
*/
void VL_LightmapMatrixFromPoints(dsurface_t *dsurf, shaderInfo_t *si,
                                 lFacet_t *f, drawVert_t *a, drawVert_t *b,
                                 drawVert_t *c) {
  int i, j;
  float t;
  ...
  if (dsurf->surfaceType != MST_PATCH) {
    ...
    if (w > LIGHTMAP_WIDTH) {
      VectorScale(f->lightmapMatrix[0], (float)LIGHTMAP_SIZE / w,
                  f->lightmapMatrix[0]);
    }

    if (h > LIGHTMAP_HEIGHT) {
      VectorScale(f->lightmapMatrix[1], (float)LIGHTMAP_SIZE / h,
                  f->lightmapMatrix[1]);
    }
    VectorSubtract(a->xyz, f->mins, delta);
    s = (DotProduct(delta, f->lightmapMatrix[0]) + dsurf->lightmapOffset[0][0] + 0.5) /
        LIGHTMAP_SIZE;
    if (fabs(s - a->lightmap[0][0]) > 0.01) {
      _printf("Bad lightmapMatrix");
    }
    t = (DotProduct(delta, f->lightmapMatrix[1]) + dsurf->lightmapOffset[0][1] + 0.5) /
        LIGHTMAP_SIZE;
    if (fabs(t - a->lightmap[0][1]) > 0.01) {
      _printf("Bad lightmapMatrix");
    }
    VectorSubtract(b->xyz, f->mins, delta);
    s = (DotProduct(delta, f->lightmapMatrix[0]) + dsurf->lightmapOffset[0][0] + 0.5) /
        LIGHTMAP_SIZE;
    if (fabs(s - b->lightmap[0][0]) > 0.01) {
      _printf("Bad lightmapMatrix");
    }
    t = (DotProduct(delta, f->lightmapMatrix[1]) + dsurf->lightmapOffset[0][1] + 0.5) /
        LIGHTMAP_SIZE;
    if (fabs(t - b->lightmap[0][1]) > 0.01) {
      _printf("Bad lightmapMatrix");
    }
    VectorSubtract(c->xyz, f->mins, delta);
    s = (DotProduct(delta, f->lightmapMatrix[0]) + dsurf->lightmapOffset[0][0] + 0.5) /
        LIGHTMAP_SIZE;
    if (fabs(s - c->lightmap[0][0]) > 0.01) {
      _printf("Bad lightmapMatrix");
    }
    t = (DotProduct(delta, f->lightmapMatrix[1]) + dsurf->lightmapOffset[0][1] + 0.5) /
        LIGHTMAP_SIZE;
    if (fabs(t - c->lightmap[0][1]) > 0.01) {
      _printf("Bad lightmapMatrix");
    }
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (i)
      al = a->lightmap[0][i] - ((float)dsurf->lightmapOffset[0][1] + 0.5) / LIGHTMAP_SIZE;
    else
      al = a->lightmap[0][i] - ((float)dsurf->lightmapOffset[0][0] + 0.5) / LIGHTMAP_SIZE;
    ...
    if (i)
      bl = b->lightmap[0][i] - ((float)dsurf->lightmapOffset[0][1] + 0.5) / LIGHTMAP_SIZE;
    else
      bl = b->lightmap[0][i] - ((float)dsurf->lightmapOffset[0][0] + 0.5) / LIGHTMAP_SIZE;
    ...
    if (i)
      cl = c->lightmap[0][i] - ((float)dsurf->lightmapOffset[0][1] + 0.5) / LIGHTMAP_SIZE;
    else
      cl = c->lightmap[0][i] - ((float)dsurf->lightmapOffset[0][0] + 0.5) / LIGHTMAP_SIZE;
    ...
    f->lightmapMatrix[i][0] = m[0][3] - f->lightmapMatrix[i][2] * m[0][2] -
                              f->lightmapMatrix[i][1] * m[0][1];
...
=============
*/
qboolean VL_PlaneFromPoints(plane_t *plane, const vec3_t a, const vec3_t b,
                            const vec3_t c) {
  vec3_t d1, d2;
...
=====================
*/
void VL_GenerateBoundaryForPoints(plane_t *boundary, plane_t *plane, vec3_t a,
                                  vec3_t b) {
  vec3_t d1;
...
=====================
*/
qboolean VL_GenerateFacetFor3Points(dsurface_t *dsurf, shaderInfo_t *si,
                                    lFacet_t *f, drawVert_t *a, drawVert_t *b,
                                    drawVert_t *c) {
  //
  vec3_t dir;
  ...
  f->lightmapCoords[0][0] = a->lightmap[0][0];
  f->lightmapCoords[0][1] = a->lightmap[0][1];
  f->lightmapCoords[1][0] = b->lightmap[0][0];
  f->lightmapCoords[1][1] = b->lightmap[0][1];
  f->lightmapCoords[2][0] = c->lightmap[0][0];
  f->lightmapCoords[2][1] = c->lightmap[0][1];

  VL_GenerateBoundaryForPoints(&f->boundaries[0], &f->plane, f->points[0],
                               f->points[1]);
  VL_GenerateBoundaryForPoints(&f->boundaries[1], &f->plane, f->points[1],
                               f->points[2]);
  VL_GenerateBoundaryForPoints(&f->boundaries[2], &f->plane, f->points[2],
                               f->points[0]);
...
qboolean VL_GenerateFacetFor4Points(dsurface_t *dsurf, shaderInfo_t *si,
                                    lFacet_t *f, drawVert_t *a, drawVert_t *b,
  ...
  for (i = 1; i < 4; i++) {
    if (!VL_PlaneFromPoints(&plane, f->points[i], f->points[(i + 1) % 4],
                            f->points[(i + 2) % 4])) {
      f->numpoints = 0;
      return qfalse;
    }
  ...
  f->lightmapCoords[2][0] = c->lightmap[0][0];
  f->lightmapCoords[2][1] = c->lightmap[0][1];
  f->lightmapCoords[3][0] = d->lightmap[0][0];
  f->lightmapCoords[3][1] = d->lightmap[0][1];
  ...
  VL_GenerateBoundaryForPoints(&f->boundaries[2], &f->plane, f->points[2],
                               f->points[3]);
  VL_GenerateBoundaryForPoints(&f->boundaries[3], &f->plane, f->points[3],
                               f->points[0]);
...
===============
*/
void VL_SphereFromBounds(vec3_t mins, vec3_t maxs, vec3_t origin,
                         float *radius) {
  vec3_t temp;
...
====================
*/
void VL_FacetsForTriangleSurface(dsurface_t *dsurf, shaderInfo_t *si,
                                 lsurfaceTest_t *test) {
  int i;
  drawVert_t *v1, *v2, *v3, *v4;
  int count;
  ...
  for (i = 0; i < test->numFacets; i++) {
    ...
    if (i != test->numFacets - 1) {
      ...
      if (i4 == i3 && i5 == i2) {
        ...
        if (VL_GenerateFacetFor4Points(dsurf, si, &test->facets[count], v1, v2,
                                       v4, v3)) {
          count++;
          i++; // skip next tri
          continue;
    ...
    if (VL_GenerateFacetFor3Points(dsurf, si, &test->facets[count], v1, v2,
                                   v3)) {
      count++;
    }
  }
...
====================
*/
void VL_FacetsForPatch(dsurface_t *dsurf, int surfaceNum, shaderInfo_t *si,
                       lsurfaceTest_t *test) {
  int i, j, x, y;
  drawVert_t *v1, *v2, *v3, *v4;
  int count, ssize;
  ...
  int widthtable[MAX_LIGHTMAP_SIZE], heighttable[MAX_LIGHTMAP_SIZE];
  ...
  if (dsurf->lightmapNum >= 0) {
    ...
    detailmesh = SubdivideMeshQuads(subdivided, ssize, LIGHTMAP_SIZE,
    ...
    if (detailmesh->width != dsurf->lightmapWidth ||
        detailmesh->height != dsurf->lightmapHeight) {
  ...
  for (i = 0; i < subdivided->width - 1; i++) {
    ...
    for (j = 0; j < subdivided->height - 1; j++) {
      ...
      if (VL_GenerateFacetFor4Points(dsurf, si, &test->facets[count], v1, v4,
                                     v3, v2)) {
        ...
        if (VL_GenerateFacetFor3Points(dsurf, si, &test->facets[count], v1, v4,
                                       v3)) {
          test->facets[count].x = x;
          test->facets[count].y = y;
          test->facets[count].width = widthtable[i];
        ...
        if (VL_GenerateFacetFor3Points(dsurf, si, &test->facets[count], v1, v3,
                                       v2)) {
          test->facets[count].x = x;
          test->facets[count].y = y;
          test->facets[count].width = widthtable[i];
...
int VL_ChopWinding(winding_t *in, plane_t *split, float epsilon) {
  ...
  for (i = 0; i < in->numpoints; i++) {
    ...
    if (neww->numpoints >= MAX_POINTS_ON_FIXED_WINDING) {
      _printf("WARNING: VL_ChopWinding -> MAX_POINTS_ON_FIXED_WINDING "
              "overflowed\n");
      return SIDE_FRONT; // can't chop -- fall back to original
    }
...
=============
*/
int VL_ChopWindingWithBrush(winding_t *w, dbrush_t *brush,
                            winding_t *outwindings, int maxout) {
  int i, res, numout;
  ...
  for (i = 0; i < brush->numSides; i++) {
    VectorCopy(dplanes[dbrushsides[brush->firstSide + i].planeNum].normal,
               plane.normal);
    ...
    if (res != SIDE_FRONT) {
      ...
      memcpy(outwindings[numout].points, back.points,
             back.numpoints * sizeof(vec3_t));
...
=============
*/
float VL_WindingAreaOutsideBrushes(winding_t *w, int *brushnums,
                                   int numbrushes) {
  ...
  for (i = 0; i < numbrushes; i++) {
    ...
    if (!(dshaders[brush->shaderNum].contentFlags &
          (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER | CONTENTS_FOG |
           CONTENTS_AREAPORTAL | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP |
           CONTENTS_CLUSTERPORTAL | CONTENTS_DONOTENTER | CONTENTS_BODY |
           CONTENTS_CORPSE | CONTENTS_TRANSLUCENT | CONTENTS_TRIGGER |
           CONTENTS_NODROP)) &&
        (dshaders[brush->shaderNum].contentFlags & CONTENTS_SOLID)) {
      numwindings[!(i & 1)] = 0;
      for (j = 0; j < numwindings[i & 1]; j++) {
        n = VL_ChopWindingWithBrush(
            &windingsbuf[i & 1][j], brush,
...
float VL_R_WindingAreaOutsideSolid(winding_t *w, vec3_t normal, int nodenum) {
  ...
  leaf = &dleafs[leafnum];
  if (leaf->cluster != -1) {
    area += VL_WindingAreaOutsideBrushes(w, &dleafbrushes[leaf->firstLeafBrush],
                                         leaf->numLeafBrushes);
  }
  return area;
}
    ...
    for (y = 0; y < ds->lightmapHeight; y++) {
      for (x = 0; x < ds->lightmapWidth; x++) {
        if (ds->surfaceType == MST_PATCH) {
          ...
          VectorCopy(mesh->verts[(y + 1) * mesh->width + x + 1].xyz,
                     w.points[2]);
          ...
          if (nostitching)
            ...
            area = VL_WindingAreaOutsideSolid(
                &w, mesh->verts[y * mesh->width + x].normal);
        } else {
          VectorMA(ds->lightmapOrigin, (float)x - LIGHTMAP_PIXELSHIFT,
                   ds->lightmapVecs[0], w.points[0]);
          VectorMA(w.points[0], (float)y - LIGHTMAP_PIXELSHIFT,
                   ds->lightmapVecs[1], w.points[0]);
          VectorMA(ds->lightmapOrigin, (float)x - LIGHTMAP_PIXELSHIFT,
                   ds->lightmapVecs[0], w.points[3]);
          VectorMA(w.points[3], (float)y - LIGHTMAP_PIXELSHIFT + 1,
                   ds->lightmapVecs[1], w.points[3]);
          VectorMA(ds->lightmapOrigin, (float)x - LIGHTMAP_PIXELSHIFT + 1,
                   ds->lightmapVecs[0], w.points[2]);
          VectorMA(w.points[2], (float)y - LIGHTMAP_PIXELSHIFT + 1,
                   ds->lightmapVecs[1], w.points[2]);
          VectorMA(ds->lightmapOrigin, (float)x - LIGHTMAP_PIXELSHIFT + 1,
                   ds->lightmapVecs[0], w.points[1]);
          VectorMA(w.points[1], (float)y - LIGHTMAP_PIXELSHIFT,
                   ds->lightmapVecs[1], w.points[1]);
          w.numpoints = 4;
          area = 0;
          for (j = 0; j < test->numFacets; j++) {
        ...
        k = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + y) *
                LIGHTMAP_WIDTH +
...
=============
*/
int VL_FindAdjacentSurface(int surfaceNum, int facetNum, vec3_t p1, vec3_t p2,
                           int *sNum, int *fNum, int *point) {
  int i, j, k;
  lsurfaceTest_t *test;
  lFacet_t *facet;
  dsurface_t *ds;
  ...
  //			lsurfaceTest[surfaceNum]->facets[facetNum].numpoints *
  // sizeof(vec3_t)); 	w.numpoints =
  // lsurfaceTest[surfaceNum]->facets[facetNum].numpoints;
  //	DebugNet_DrawWinding(&w, 2);
  for (i = 0; i < numDrawSurfaces; i++) {
    if (i == surfaceNum)
    ...
    if (fabs(dir[0]) > test->radius || fabs(dir[1]) > test->radius ||
    ...
    for (j = 0; j < test->numFacets; j++) {
      ...
      for (k = 0; k < facet->numpoints; k++) {
        fp1 = facet->points[k];
        if (fabs(p2[0] - fp1[0]) < 0.1 && fabs(p2[1] - fp1[1]) < 0.1 &&
            fabs(p2[2] - fp1[2]) < 0.1) {
          fp2 = facet->points[(k + 1) % facet->numpoints];
          if (fabs(p1[0] - fp2[0]) < 0.1 && fabs(p1[1] - fp2[1]) < 0.1 &&
              fabs(p1[2] - fp2[2]) < 0.1) {
            //	memcpy(w.points, facet->points, facet->numpoints *
            // sizeof(vec3_t)); 	w.numpoints = facet->numpoints;
  ...
  for (i = 0; i < numDrawSurfaces; i++) {
    ...
    for (j = 0; j < test->numFacets; j++) {
      ...
      for (k = 0; k < facet1->numpoints; k++) {
        ...
        coords1[1][0] = facet1->lightmapCoords[(k + 1) % facet1->numpoints][0] *
                        LIGHTMAP_SIZE;
        coords1[1][1] = facet1->lightmapCoords[(k + 1) % facet1->numpoints][1] *
                        LIGHTMAP_SIZE;
        ...
        if (coords1[0][0] - coords1[1][0] == 0) {
          ...
          coords1[0][0] += s < 0 ? 1 : -1;
          coords1[1][0] += s < 0 ? 1 : -1;
          if (coords1[0][0] < ds1->lightmapOffset[0][0] ||
              coords1[0][0] >= ds1->lightmapOffset[0][0] + ds1->lightmapWidth) {
            coords1[0][0] += s < 0 ? -1 : 1;
            coords1[1][0] += s < 0 ? -1 : 1;
          }
          ...
          coords1[0][1] += t < 0 ? 1 : -1;
          coords1[1][1] += t < 0 ? 1 : -1;
          if (coords1[0][1] < ds1->lightmapOffset[0][1] ||
              coords1[0][1] >= ds1->lightmapOffset[0][1] + ds1->lightmapHeight) {
            coords1[0][1] += t < 0 ? -1 : 1;
            coords1[1][1] += t < 0 ? -1 : 1;
          }
        ...
        if (xinc1 != 0 && yinc1 != 0)
          continue;
        //
        if (!VL_FindAdjacentSurface(i, j, p[0], p[1], &surfaceNum, &facetNum,
                                    &point))
          continue;
        //
        ...
        coords2[0][0] =
            facet2->lightmapCoords[(point + 1) % facet2->numpoints][0] *
            LIGHTMAP_SIZE;
        coords2[0][1] =
            facet2->lightmapCoords[(point + 1) % facet2->numpoints][1] *
            LIGHTMAP_SIZE;
        ...
        //
        while (1) {
          k1 = (ds1->lightmapNum[0] * LIGHTMAP_HEIGHT + y1) * LIGHTMAP_WIDTH + x1;
          k2 = (ds2->lightmapNum[0] * LIGHTMAP_HEIGHT + ((int)y2)) *
                   LIGHTMAP_WIDTH +
               ((int)x2);
          ...
          x2 += xinc2;
          y2 += yinc2;
          if (x2 < ds2->lightmapOffset[0][0])
            x2 = ds2->lightmapOffset[0][0];
          if (x2 >= ds2->lightmapOffset[0][0] + ds2->lightmapWidth)
            x2 = ds2->lightmapOffset[0][0] + ds2->lightmapWidth - 1;
          if (y2 < ds2->lightmapOffset[0][1])
            y2 = ds2->lightmapOffset[0][1];
          if (y2 >= ds2->lightmapOffset[0][1] + ds2->lightmapHeight)
            y2 = ds2->lightmapOffset[0][1] + ds2->lightmapHeight - 1;
        }
      }
    }
  }
...
void VL_FixLightmapEdges(void) {
  ...
  float color[3];
  float *ptr;
  byte filled[(MAX_LIGHTMAP_SIZE + 1) * (MAX_LIGHTMAP_SIZE + 1) / 8];
    ...
    for (x = 0; x < width; x++) {
      for (y = 0; y < height; y++) {
        ...
        if (lightmappixelarea[k] > lightmap_edge_epsilon) {
          index = (ds->lightmapOffset[0][1] + y) * LIGHTMAP_WIDTH + ds->lightmapOffset[0][0] + x;
    ...
    for (y = 0; y < height; y++) {
      ...
      for (x = 0; x < width; x++) {
        ...
        if (pos == -2) {
          ...
          if (filled[index >> 3] & (1 << (index & 7))) {
            bottom = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + y) *
                         LIGHTMAP_WIDTH +
                     ds->lightmapOffset[0][0] + pos;
            top = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + y) *
                      LIGHTMAP_WIDTH +
                  ds->lightmapOffset[0][0] + x;
            for (j = 0; j < (x - pos + 1) / 2; j++) {
              k = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + y) *
                      LIGHTMAP_WIDTH +
                  ds->lightmapOffset[0][0] + pos + j + 1;
              index = (ds->lightmapOffset[0][1] + y) * LIGHTMAP_WIDTH + ds->lightmapOffset[0][0] +
                      pos + j + 1;
              filled[index >> 3] |= 1 << (index & 7);
    ...
    for (x = 0; x < width; x++) {
      ...
      for (y = 0; y < height; y++) {
        ...
        if (pos == -2) {
          ...
          if (filled[index >> 3] & (1 << (index & 7))) {
            bottom = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + pos) *
                         LIGHTMAP_WIDTH +
                     ds->lightmapOffset[0][0] + x;
            top = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + y) *
                      LIGHTMAP_WIDTH +
            ...
            for (j = 0; j < (y - pos + 1) / 2; j++) {
              k = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + pos + j +
                   1) *
                      LIGHTMAP_WIDTH +
                  ds->lightmapOffset[0][0] + x;
              index = (ds->lightmapOffset[0][1] + pos + j + 1) * LIGHTMAP_WIDTH +
                      ds->lightmapOffset[0][0] + x;
              filled[index >> 3] |= 1 << (index & 7);
              (lightFloats + k * 3)[0] = (lightFloats + top * 3)[0];
              (lightFloats + k * 3)[1] = (lightFloats + top * 3)[1];
              (lightFloats + k * 3)[2] = (lightFloats + top * 3)[2];
              k = (ds->lightmapNum[0] * LIGHTMAP_HEIGHT + ds->lightmapOffset[0][1] + y - j -
                   1) *
                      LIGHTMAP_WIDTH +
                  ds->lightmapOffset[0][0] + x;
              index = (ds->lightmapOffset[0][1] + y - j - 1) * LIGHTMAP_WIDTH +
                      ds->lightmapOffset[0][0] + x;
              filled[index >> 3] |= 1 << (index & 7);
              (lightFloats + k * 3)[0] = (lightFloats + bottom * 3)[0];
              (lightFloats + k * 3)[1] = (lightFloats + bottom * 3)[1];
...
void VL_ShiftPatchLightmaps(void) {
  ...
  for (i = 0; i < numDrawSurfaces; i++) {
    ...
    for (j = 0; j < ds->patchHeight * ds->patchWidth; j++) {
      verts[j].lightmap[0][0] += 0.5 / LIGHTMAP_WIDTH;
      verts[j].lightmap[0][1] += 0.5 / LIGHTMAP_HEIGHT;
    }
    ds->lightmapHeight++;
    ds->lightmapWidth++;
  }
...
=============
*/
int VL_PointInLeafnum(vec3_t point) { return VL_PointInLeafnum_r(point, 0); }
...
=============
*/
void VL_LightSurfaceWithVolume(int surfaceNum, int facetNum, vlight_t *light,
...
int VL_PointInsideLightVolume(vec3_t point, lightvolume_t *volume)
{
        ...
        for (i = 0; i < volume->numplanes; i++)
        {
                d = DotProduct(volume->planes[i].normal, point) -
volume->planes[i].dist; if (d < 0) return qfalse;
        }
        return qtrue;
}
...
lightvolume_t *volume )
{
        ...
        for ( i = 0 ; i < sampleWidth; i++ ) {
                for ( j = 0 ; j < sampleHeight; j++ ) {
                        ...
                        if ( ds->patchWidth ) {
                                numPositions = 9;
                                VectorCopy( mesh->verts[j*mesh->width+i].normal,
normal );
                                // VectorNormalize( normal, normal );
                                // push off of the curve a bit
                                VectorMA( mesh->verts[j*mesh->width+i].xyz, 1,
normal, base );

//				MakeNormalVectors( normal, lightmapVecs[0],
lightmapVecs[1] ); } else { numPositions = 9; for ( k = 0 ; k < 3 ; k++ ) {
...
lightmapVecs[1][k];
                        ...
                        if ( light->atten_disttype == LDAT_LINEAR ) {
                                add = angle * light->photons * lightLinearScale
- dist; if ( add < 0 ) { add = 0;
                                }
                        } else {
                        ...
                        {
                                k = ( ds->lightmapNum[0] * LIGHTMAP_HEIGHT +
ds->lightmapOffset[0][1] + j)
                                        * LIGHTMAP_WIDTH + ds->lightmapOffset[0][0] + i;
                                ptr = lightBytes + k*3;
...
=============
*/
void VL_GetFilter(vlight_t *light, lightvolume_t *volume, vec3_t lmp,
                  vec3_t filter) {
  lFacet_t *facet;
  lsurfaceTest_t *test;
  ...
  if (light->type == LIGHT_SURFACEDIRECTED) {
    ...
    d = DotProduct(lmp, light->normal) -
        DotProduct(light->normal, light->w.points[0]);
...
void VL_LightSurfaceWithVolume(int surfaceNum, int facetNum, vlight_t *light,
  ...
  int min_x[MAX_LIGHTMAP_SIZE + 2], max_x[MAX_LIGHTMAP_SIZE + 2];
  int min_y, max_y, k, x, y, n;
  float *color, distscale;
  float d, add, angle, dist, area, insidearea,
      coords[MAX_POINTS_ON_WINDING + 1][2];
  mesh_t *mesh;
  byte polygonedges[(MAX_LIGHTMAP_SIZE + 1) * (MAX_LIGHTMAP_SIZE + 1) / 8];
  ...
  if (ds->surfaceType == MST_PATCH) {
    ...
    min_y = ds->lightmapOffset[0][1] + facet->y;
    max_y = ds->lightmapOffset[0][1] + facet->y + facet->height - 1;
    for (y = min_y; y <= max_y; y++) {
      min_x[y] = ds->lightmapOffset[0][0] + facet->x;
      max_x[y] = ds->lightmapOffset[0][0] + facet->x + facet->width - 1;
    ...
    for (i = 0; i < w.numpoints; i++) {
      ...
      if (ds->surfaceType != MST_PATCH) {
        ...
        s = DotProduct(delta, facet->lightmapMatrix[0]) + ds->lightmapOffset[0][0] + 0.5;
        t = DotProduct(delta, facet->lightmapMatrix[1]) + ds->lightmapOffset[0][1] + 0.5;
        if (s >= LIGHTMAP_SIZE)
          s = LIGHTMAP_SIZE - 0.5;
        if (s < 0)
          s = 0;
        ...
        s = DotProduct(w.points[i], facet->lightmapMatrix[0]) +
            facet->lightmapMatrix[0][3];
        t = DotProduct(w.points[i], facet->lightmapMatrix[1]) +
            facet->lightmapMatrix[1][3];

        s = s - floor(s);
        t = t - floor(t);

        coords[i][0] = ds->lightmapOffset[0][0] + s * LIGHTMAP_SIZE; // + 0.5;
        coords[i][1] = ds->lightmapOffset[0][1] + t * LIGHTMAP_SIZE; // + 0.5;

        if (coords[i][0] >= LIGHTMAP_SIZE)
          coords[i][0] -= LIGHTMAP_SIZE;
        if (coords[i][1] >= LIGHTMAP_SIZE)
          coords[i][1] -= LIGHTMAP_SIZE;
        if (coords[i][0] < ds->lightmapOffset[0][0])
          coords[i][0] = ds->lightmapOffset[0][0];
        if (coords[i][1] < ds->lightmapOffset[0][1])
          coords[i][1] = ds->lightmapOffset[0][1];
      }
      x = coords[i][0];
      y = coords[i][1];
      if (x < ds->lightmapOffset[0][0] || x >= LIGHTMAP_SIZE)
        _printf("VL_LightSurfaceWithVolume: x outside lightmap\n");
      if (y < ds->lightmapOffset[0][1] || y >= LIGHTMAP_SIZE)
  ...
  for (y = min_y; y <= max_y; y++) {
    for (x = min_x[y]; x <= max_x[y]; x++) {
      if (ds->surfaceType == MST_PATCH) {
        ...
        VectorCopy(
            mesh->verts[(y - ds->lightmapOffset[0][1]) * mesh->width + x - ds->lightmapOffset[0][0]]
                .xyz,
            base);
        VectorCopy(
            mesh->verts[(y - ds->lightmapOffset[0][1]) * mesh->width + x - ds->lightmapOffset[0][0]]
        ...
        VectorMA(ds->lightmapOrigin, (float)x - ds->lightmapOffset[0][0],
                 ds->lightmapVecs[0], base);
        VectorMA(base, (float)y - ds->lightmapOffset[0][1], ds->lightmapVecs[1], base);
        VectorCopy(facet->plane.normal, normal);
      }
      if (light->type == LIGHT_POINTSPOT) {
        ...
        float formFactor = PointToPolygonFormFactor(base, normal, &light->w);
        if (formFactor < 0 && light->twosided) {
          formFactor = -formFactor;
        }
        add = CalculateFalloff(formFactor);
        if (add <= 0) {
          continue;
        }
        ...
        // VectorInverse(dir);
        //  project the light map pixel origin onto the area light source plane
        d = DotProduct(base, light->normal) -
            DotProduct(light->normal, light->w.points[0]);
      ...
      // if the lightmap pixel is a patch, we need to calculate the area of the
      // lightmap pixel
      if (ds->surfaceType == MST_PATCH) {
        mesh = test->detailMesh;
        if (y - ds->lightmapOffset[0][1] >= mesh->height - 1)
          _printf("y outside mesh\n");
        if (x - ds->lightmapOffset[0][0] >= mesh->width - 1)
          _printf("x outside mesh\n");
        VectorCopy(
            mesh->verts[(y - ds->lightmapOffset[0][1]) * mesh->width + x - ds->lightmapOffset[0][0]]
                .xyz,
            w.points[0]);
        VectorCopy(mesh->verts[(y + 1 - ds->lightmapOffset[0][1]) * mesh->width + x -
                               ds->lightmapOffset[0][0]]
                       .xyz,
                   w.points[1]);
        VectorCopy(mesh->verts[(y + 1 - ds->lightmapOffset[0][1]) * mesh->width + x + 1 -
                       ...
                       .xyz,
                   w.points[2]);
        VectorCopy(mesh->verts[(y - ds->lightmapOffset[0][1]) * mesh->width + x + 1 -
                               ds->lightmapOffset[0][0]]
                       .xyz,
                   w.points[3]);
        w.numpoints = 4;
        ...
        VectorMA(ds->lightmapOrigin,
                 (float)x - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][0],
                 ds->lightmapVecs[0], w.points[0]);
        VectorMA(w.points[0], (float)y - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][1],
                 ds->lightmapVecs[1], w.points[0]);
        VectorMA(ds->lightmapOrigin,
        ...
        VectorMA(ds->lightmapOrigin,
                 (float)x + LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][0],
                 ds->lightmapVecs[0], w.points[2]);
        VectorMA(w.points[2], (float)y + LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][1],
                 ds->lightmapVecs[1], w.points[2]);
        VectorMA(ds->lightmapOrigin,
                 (float)x + LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][0],
                 ds->lightmapVecs[0], w.points[1]);
        VectorMA(w.points[1], (float)y - LIGHTMAP_PIXELSHIFT - ds->lightmapOffset[0][1],
                 ds->lightmapVecs[1], w.points[1]);
        w.numpoints = 4;
      }
      // if the lightmap pixel is partly inside the light volume
      if (polygonedges[(y * LIGHTMAP_SIZE + x) >> 3] &
          (1 << ((y * LIGHTMAP_SIZE + x) & 7))) {
        // if the lightmap pixel is a patch, we need to calculate the area of the
        // lightmap pixel
        // area = WindingArea(&w);
      ...
      // get the light filter from all the translucent surfaces the light volume
      // went through
...
=============
*/
int VL_SplitLightVolume(lightvolume_t *volume, lightvolume_t *back,
                        plane_t *split, float epsilon) {
  lightvolume_t f, b;
  vec_t dists[128];
  int sides[128];
  int counts[3];
  ...
  for (i = 0; i < volume->numplanes; i++) {
    ...
    if (f.numplanes >= MAX_POINTS_ON_FIXED_WINDING) {
      _printf("WARNING: VL_SplitLightVolume -> MAX_POINTS_ON_FIXED_WINDING "
              "overflowed\n");
...
=============
*/
void VL_PlaneForEdgeToWinding(vec3_t p1, vec3_t p2, winding_t *w,
                              int windingonfront, plane_t *plane) {
  int i, j;
  float length, d;
  vec3_t v1, v2;
...
=============
*/
void VL_R_FloodLight(vlight_t *light, lightvolume_t *volume, int cluster,
                     int firstportal);
...
void VL_R_CastLightAtSurface(vlight_t *light, lightvolume_t *volume) {
  ...
  VL_LightSurfaceWithVolume(volume->surfaceNum, volume->facetNum, light,
                            volume);
  //
  test = lsurfaceTest[volume->surfaceNum];
  // if this is not a translucent surface
  if (!(test->shader->surfaceFlags & SURF_ALPHASHADOW) &&
      !(test->shader->contents & CONTENTS_TRANSLUCENT))
    return;
  //
  if (volume->numtransFacets >= MAX_TRANSLUCENTFACETS)
    Error("a light valume went through more than %d translucent facets",
...
int VL_R_SplitLightVolume(vlight_t *light, lightvolume_t *volume,
                          plane_t *split, int cluster, int firstportal) {
  lightvolume_t back;
  int res;
  ...
  if (res == 2) {
    memcpy(back.clusterTested, volume->clusterTested,
           sizeof(back.clusterTested));
    ...
    if (volume->numtransFacets > 0) {
      ...
      memcpy(back.transSurfaces, volume->transSurfaces,
             sizeof(back.transSurfaces));
    }
...
void VL_R_FloodLight(vlight_t *light, lightvolume_t *volume, int cluster,
  ...
  // if the first portal is not zero then we've checked all occluders in this
  // leaf already
  if (firstportal == 0) {
    ...
    for (i = 0; i < leafs[cluster].numSurfaces; i++) {
      ...
      for (j = 0; j < test->numFacets; j++) {
        ...
        facet = &test->facets[j];
        //
        //	memcpy(winding.points, facet->points, sizeof(vec3_t) *
        // facet->numpoints); 	winding.numpoints = facet->numpoints;
        ...
        if (!test->patch && !test->trisoup) {
          if (volume->type == VOLUME_NORMAL) {
            ...
            d = DotProduct(light->origin, facet->plane.normal) -
                facet->plane.dist;
            if (d < 0) {
              // NOTE: this doesn't work too great because of sometimes very bad
              // tesselation
              //		of surfaces that are supposed to be flat
              // FIXME: to work around this problem we should make sure that all
              // facets
              //		created from planar surfaces use the
              // lightmapVecs normal vector
              /*
              if ( !test->shader->twoSided )
              {
                      // skip all other facets of this surface as well because
              they are in the same plane for (k = 0; k < test->numFacets; k++)
                      {
                              facet = &test->facets[k];
                              volume->facetTested[facet->num >> 3] |= 1 <<
              (facet->num & 7);
                      }
              }*/
            ...
            // FIXME: if all light source winding points are at the back of the
            // facet
        ...
        // check if this facet is totally or partly in front of the volume end
        plane for (k = 0; k < facet->numpoints; k++)
        {
                d = DotProduct(volume->endplane.normal, facet->points[k]) -
        volume->endplane.dist; if (d > ON_EPSILON) break;
        }
        ...
        */
        ...
        if (backfaceculled) {
          ...
          // flip the winding
          for (k = 0; k < facet->numpoints; k++)
            VectorCopy(facet->points[k],
                       winding.points[facet->numpoints - k - 1]);
          winding.numpoints = facet->numpoints;
        } else {
          memcpy(winding.points, facet->points,
                 sizeof(vec3_t) * facet->numpoints);
          winding.numpoints = facet->numpoints;
        }
        //
        if (!testculled) {
          testculled = qtrue;
          // fast check if the surface sphere is totally behind the volume end
          // plane
          d = DotProduct(volume->endplane.normal, test->origin) -
              volume->endplane.dist;
          if (d < -test->radius) {
          ...
          for (k = 0; k < volume->numplanes; k++) {
            d = DotProduct(volume->planes[k].normal, test->origin) -
                volume->planes[k].dist;
            if (d < -test->radius) {
              for (k = 0; k < test->numFacets; k++) {
                facet = &test->facets[k];
                volume->facetTested[facet->num >> 3] |= 1 << (facet->num & 7);
        ...
        // NOTE: we have to chop the facet winding with the volume end plane
        // because 		the faces in Q3 are not stitched together nicely
        res = VL_ChopWinding(&winding, &volume->endplane, 0.01);
        // if the facet is on or at the back of the volume end plane
        if (res == SIDE_BACK || res == SIDE_ON)
          continue;
        // check if the facet winding is totally or partly inside the light
        // volume
        ...
        for (k = 0; k < winding.numpoints; k++) {
          if (volume->type == VOLUME_DIRECTED) {
            VectorSubtract(winding.points[(k + 1) % winding.numpoints],
                           winding.points[k], dir1);
  ...
  for (i = firstportal; i < leaf->numportals; i++) {
    ...
    if (volume->type == VOLUME_NORMAL) {
      ...
      // FIXME: if all light source winding points are at the back of this
      // portal
      //			plane then there's no need to flood through
    }
    // check if this portal is totally or partly in front of the volume end
    // plane fast check with portal sphere
    d = DotProduct(volume->endplane.normal, p->origin) - volume->endplane.dist;
    if (d < -p->radius)
      continue;
    for (j = 0; j < p->winding->numpoints; j++) {
      d = DotProduct(volume->endplane.normal, p->winding->points[j]) -
          volume->endplane.dist;
      if (d > -0.01)
        break;
    }
    ...
    for (k = 0; k < p->winding->numpoints; k++) {
      if (volume->type == VOLUME_DIRECTED) {
        VectorSubtract(p->winding->points[(k + 1) % p->winding->numpoints],
                       p->winding->points[k], dir1);
        CrossProduct(light->normal, dir1, plane.normal);
        VectorNormalize(plane.normal, plane.normal);
...
=============
*/
void VL_R_SubdivideDirectedAreaLight(vlight_t *light, int nodenum,
                                     winding_t *w) {
  int leafnum, res;
  dnode_t *node;
  dplane_t *plane;
...
void VL_FloodLight(vlight_t *light) {
  ...
  case LIGHT_POINTRADIAL: {
    ...
    for (i = 0; i < 3; i++) {
      ...
      for (j = -1; j <= 1; j += 2) {
        ...
        volume.numplanes = 0;
        for (k = 0; k < 4; k++) {
          volume.points[volume.numplanes][i] =
              light->origin[i] + j * windingdist;
          volume.points[volume.numplanes][(i + 1) % 3] =
              light->origin[(i + 1) % 3] + dir[0][k] * windingdist;
          volume.points[volume.numplanes][(i + 2) % 3] =
              light->origin[(i + 2) % 3] + dir[1][k] * windingdist;
          volume.numplanes++;
        }
        if (j >= 0) {
        ...
        for (k = 0; k < volume.numplanes; k++) {
          VL_PlaneFromPoints(&volume.planes[k], light->origin,
                             volume.points[(k + 1) % volume.numplanes],
                             volume.points[k]);
        }
        ...
        volume.endplane.dist = DotProduct(
            volume.endplane.normal,
            temp); // DotProduct(volume.endplane.normal, volume.points[0]);
        volume.farplane = volume.endplane;
  ...
  case LIGHT_POINTSPOT: {
    ...
    if (dist > windingdist)
      windingdist = dist;
    // take 8 times the cone radius because the spotlight also lights outside
    // the cone
    ...
    for (a = step / 2; a < 360 + step / 2; a += step) {
      RotatePointAroundVector(volume.points[volume.numplanes], light->normal, p,
                              a);
      VectorAdd(light->origin, volume.points[volume.numplanes],
                volume.points[volume.numplanes]);
      VectorMA(volume.points[volume.numplanes], windingdist, light->normal,
               volume.points[volume.numplanes]);
      volume.numplanes++;
    }
    for (i = 0; i < volume.numplanes; i++) {
      VL_PlaneFromPoints(&volume.planes[i], light->origin,
                         volume.points[(i + 1) % volume.numplanes],
                         volume.points[i]);
    }
    ...
    for (i = -1; i <= 1; i += 2) {
      for (j = -1; j <= 1; j += 2) {
        for (n = 0; n < 2; n++) {
          memset(&volume, 0, sizeof(lightvolume_t));
          volume.numplanes = 3;
          VectorMA(light->origin, i * windingdist, vecs[0],
                   volume.points[(i == j) == n]);
          VectorMA(light->origin, j * windingdist, vecs[1],
                   volume.points[(i != j) == n]);
          ...
          VL_PlaneFromPoints(&volume.endplane, volume.points[0],
                             volume.points[1], volume.points[2]);
          VectorMA(light->origin, dist, light->normal, temp);
          volume.endplane.dist = DotProduct(volume.endplane.normal, temp);
          if (DotProduct(light->origin, volume.endplane.normal) -
                  volume.endplane.dist >
              0)
            break;
        }
...
=============
*/
void VL_FloodLightThread(int num) { VL_FloodLight(vlights[num]); }
...
void VL_SurfaceRadiosity(int num) {
  ...
  for (x = 0; x < ds->lightmapWidth; x++) {
    for (y = 0; y < ds->lightmapHeight; y++) {
      ...
      // ok this is crap
      vlight.photons = VectorLength(color) * 0.05 * lightPointScale /
                       (area * radiosity_scale);
...
void VL_SubdivideAreaLight(shaderInfo_t *ls, winding_t *w, vec3_t normal,
  ...
  VectorScale(ls->color, value * lightFormFactorValueScale * lightAreaScale,
              dl->emitColor);
  //
...
winding_t *VL_WindingForBrushSide(dbrush_t *brush, int side, winding_t *w) {
  ...
  VectorCopy(dplanes[dbrushsides[brush->firstSide + side].planeNum].normal,
             plane.normal);
  VectorInverse(plane.normal);
...
void VL_CreateSkyLights(void) {
  ...
  dbrushside_t *s;
  vlight_t *dl;
  vec3_t sunColor = {0, 0, 0}, sunDir = {0.45, 0.3, 0.9};
  float d;
