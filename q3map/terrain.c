...
================
*/
shaderInfo_t *ShaderForLayer(int minlayer, int maxlayer,
...
byte *LoadAlphaMap(int *num_layers, int *alphawidth, int *alphaheight) {
  ...
  if (!Q_stricmp(ext, "tga")) {
    Load32BitImage(ExpandGamePath(alphamapname), (unsigned int **)&alphamap32,
                   &width, &height);
...
static void SideAsTriFan(terrainSurf_t *surf, int *index, int num) {
  ...
  if (surf->numIndexes + num * 3 > surf->maxIndexes) {
    ...
    surf->indexes =
        realloc(surf->indexes, surf->maxIndexes * sizeof(*surf->indexes));
  }
...
static void SideAsTristrip(terrainSurf_t *surf, int *index, int num) {
  ...
  memcpy(surf->indexes + surf->numIndexes, indices,
         ni * sizeof(*surf->indexes));
  surf->numIndexes += ni;
}
...
void CreateTerrainSurface(terrainSurf_t *surf, shaderInfo_t *shader) {
  ...
  memcpy(newsurf->indexes, surf->indexes,
         surf->numIndexes * sizeof(*newsurf->indexes));
...
================
*/
void EmitTerrainVerts(side_t *side, terrainSurf_t *surf, int maxlayer,
  ...
  for (i = 0; i < side->winding->numpoints; i++) {
    ...
    if (projecttexture) {
      vert->st[0] =
          (vecs[0][3] + DotProduct(vecs[0], vert->xyz)) / surf->shader->width;
      vert->st[1] =
          (vecs[1][3] + DotProduct(vecs[1], vert->xyz)) / surf->shader->height;
    } else {
      vert->st[0] = (side->vecs[0][3] + DotProduct(side->vecs[0], vert->xyz)) /
                    surf->shader->width;
      vert->st[1] = (side->vecs[1][3] + DotProduct(side->vecs[1], vert->xyz)) /
                    surf->shader->height;
    }
    ...
    if (j != surf->numVerts) {
      ...
      if (surf->numVerts >= surf->maxVerts) {
        surf->maxVerts += GROW_VERTS;
        surf->verts =
            realloc(surf->verts, surf->maxVerts * sizeof(*surf->verts));
      }
    }
  }
...
terrainSurf_t *SurfaceForShader(shaderInfo_t *shader, int x, int y) {
  ...
  if (lastSurface && (lastSurface->shader == shader) && (lastSurface->x == x) &&
      (lastSurface->y == y)) {
    return lastSurface;
  }
  ...
  for (i = 0; i < numsurfaces; i++, lastSurface++) {
    if ((lastSurface->shader == shader) && (lastSurface->x == x) &&
        (lastSurface->y == y)) {
      return lastSurface;
    }
  }
  ...
  if (numsurfaces >= maxsurfaces) {
    ...
    memset(surfaces + numsurfaces + 1, 0,
           (maxsurfaces - numsurfaces - 1) * sizeof(*surfaces));
  }
...
void SetTerrainTextures(void) {
  ...
  for (brush = mapent->brushes; brush != NULL; brush = brush->next) {
    ...
    for (side = brush->sides; side < &brush->sides[brush->numsides]; side++) {
      ...
      if (((side->surfaceFlags | side->shaderInfo->surfaceFlags) &
           SURF_NODRAW) &&
...
void Terrain_GetTriangles(terrainMesh_t *pm, int x, int y,
                          terrainVert_t **verts) {
  if ((x + y) & 1) {
    // first tri
...
================
*/
void EmitTerrainVerts2(terrainSurf_t *surf, terrainVert_t **verts,
                       int alpha[3]) {
  int i;
  int j;
  drawVert_t *vert;
...
void AddBrushSide(vec3_t v1, vec3_t v2, vec3_t v3,
...
void MakeBrushFromTriangle(vec3_t v1, vec3_t v2, vec3_t v3,
                           shaderInfo_t *terrainShader) {
  bspbrush_t *b;
  vec3_t d1;
  vec3_t d2;
...
void MakeTerrainIntoBrushes(terrainMesh_t *tm) {
  ...
  for (y = 0; y < tm->height - 1; y++) {
    for (x = 0; x < tm->width - 1; x++) {
      ...
      MakeBrushFromTriangle(verts[index[0]].xyz, verts[index[1]].xyz,
                            verts[index[2]].xyz, terrainShader);
      MakeBrushFromTriangle(verts[index[3]].xyz, verts[index[4]].xyz,
                            verts[index[5]].xyz, terrainShader);
    }
  }
}
