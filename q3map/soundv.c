...
=============
*/
int VS_SplitWinding(winding_t *in, winding_t *back, plane_t *split,
                    float epsilon) {
  vec_t dists[128];
  int sides[128];
  int counts[3];
  ...
  for (i = 0; i < in->numpoints; i++) {
    ...
    if (neww->numpoints >= MAX_POINTS_ON_FIXED_WINDING) {
      _printf("WARNING: VS_SplitWinding -> MAX_POINTS_ON_FIXED_WINDING "
              "overflowed\n");
...
=====================
*/
void VS_TextureMatrixFromPoints(lFacet_t *f, drawVert_t *a, drawVert_t *b,
                                drawVert_t *c) {
  int i, j;
  float t;
  float m[3][4];
...
=====================
*/
void VS_LightmapMatrixFromPoints(dsurface_t *dsurf, shaderInfo_t *si,
...
=============
*/
qboolean VS_PlaneFromPoints(plane_t *plane, const vec3_t a, const vec3_t b,
                            const vec3_t c) {
  vec3_t d1, d2;
...
=====================
*/
void VS_GenerateBoundaryForPoints(plane_t *boundary, plane_t *plane, vec3_t a,
                                  vec3_t b) {
  vec3_t d1;
...
=====================
*/
qboolean VS_GenerateFacetFor3Points(dsurface_t *dsurf, shaderInfo_t *si,
                                    lFacet_t *f, drawVert_t *a, drawVert_t *b,
                                    drawVert_t *c) {
  //
  vec3_t dir;
  ...
  VS_GenerateBoundaryForPoints(&f->boundaries[0], &f->plane, f->points[0],
                               f->points[1]);
  VS_GenerateBoundaryForPoints(&f->boundaries[1], &f->plane, f->points[1],
                               f->points[2]);
  VS_GenerateBoundaryForPoints(&f->boundaries[2], &f->plane, f->points[2],
                               f->points[0]);
...
qboolean VS_GenerateFacetFor4Points(dsurface_t *dsurf, shaderInfo_t *si,
                                    lFacet_t *f, drawVert_t *a, drawVert_t *b,
  ...
  for (i = 1; i < 4; i++) {
    if (!VS_PlaneFromPoints(&plane, f->points[i], f->points[(i + 1) % 4],
                            f->points[(i + 2) % 4])) {
      f->numpoints = 0;
      return qfalse;
    }
  ...
  VS_GenerateBoundaryForPoints(&f->boundaries[2], &f->plane, f->points[2],
                               f->points[3]);
  VS_GenerateBoundaryForPoints(&f->boundaries[3], &f->plane, f->points[3],
                               f->points[0]);
...
===============
*/
void VS_SphereFromBounds(vec3_t mins, vec3_t maxs, vec3_t origin,
                         float *radius) {
  vec3_t temp;
...
====================
*/
void VS_FacetsForTriangleSurface(dsurface_t *dsurf, shaderInfo_t *si,
                                 lsurfaceTest_t *test) {
  int i;
  drawVert_t *v1, *v2, *v3, *v4;
  int count;
  ...
  for (i = 0; i < test->numFacets; i++) {
    ...
    if (i != test->numFacets - 1) {
      ...
      if (i4 == i3 && i5 == i2) {
        ...
        if (VS_GenerateFacetFor4Points(dsurf, si, &test->facets[count], v1, v2,
                                       v4, v3)) {
          count++;
          i++; // skip next tri
          continue;
    ...
    if (VS_GenerateFacetFor3Points(dsurf, si, &test->facets[count], v1, v2,
                                   v3)) {
      count++;
    }
  }
...
====================
*/
void VS_FacetsForPatch(dsurface_t *dsurf, int surfaceNum, shaderInfo_t *si,
                       lsurfaceTest_t *test) {
  int i, j, x, y;
  drawVert_t *v1, *v2, *v3, *v4;
  int count, ssize;
  ...
  for (i = 0; i < subdivided->width - 1; i++) {
    ...
    for (j = 0; j < subdivided->height - 1; j++) {
      ...
      if (VS_GenerateFacetFor4Points(dsurf, si, &test->facets[count], v1, v4,
                                     v3, v2)) {
        ...
        if (VS_GenerateFacetFor3Points(dsurf, si, &test->facets[count], v1, v4,
                                       v3)) {
          test->facets[count].x = x;
          test->facets[count].y = y;
          test->facets[count].width = widthtable[i];
        ...
        if (VS_GenerateFacetFor3Points(dsurf, si, &test->facets[count], v1, v3,
                                       v2)) {
          test->facets[count].x = x;
          test->facets[count].y = y;
          test->facets[count].width = widthtable[i];
...
int VS_ChopWinding(winding_t *in, plane_t *split, float epsilon) {
  ...
  for (i = 0; i < in->numpoints; i++) {
    ...
    if (neww->numpoints >= MAX_POINTS_ON_FIXED_WINDING) {
      _printf("WARNING: VS_ChopWinding -> MAX_POINTS_ON_FIXED_WINDING "
              "overflowed\n");
      return SIDE_FRONT; // can't chop -- fall back to original
    }
...
=============
*/
int VS_ChopWindingWithBrush(winding_t *w, dbrush_t *brush,
...
=============
*/
float VS_WindingAreaOutsideBrushes(winding_t *w, int *brushnums,
                                   int numbrushes) {
...
float VS_R_WindingAreaOutsideSolid(winding_t *w, vec3_t normal, int nodenum) {
  ...
  leaf = &dleafs[leafnum];
  if (leaf->cluster != -1) {
    area += VS_WindingAreaOutsideBrushes(w, &dleafbrushes[leaf->firstLeafBrush],
                                         leaf->numLeafBrushes);
  }
  return area;
}
      ...
      for (x = 0; x < ds->lightmapWidth; x++) {
        if (ds->surfaceType == MST_PATCH) {
          ...
          if (nostitching)
            ...
            area = VS_WindingAreaOutsideSolid(
...
=============
*/
int VS_FindAdjacentSurface(int surfaceNum, int facetNum, vec3_t p1, vec3_t p2,
    ...
    for (j = 0; j < test->numFacets; j++) {
      ...
      for (k = 0; k < facet1->numpoints; k++) {
        ...
        if (xinc1 != 0 && yinc1 != 0)
          continue;
        //
        if (!VS_FindAdjacentSurface(i, j, p[0], p[1], &surfaceNum, &facetNum,
                                    &point))
          continue;
        //
...
=============
*/
int VS_PointInLeafnum(vec3_t point) { return VS_PointInLeafnum_r(point, 0); }
...
=============
*/
void VS_LightSurfaceWithVolume(int surfaceNum, int facetNum, vsound_t *light,
...
=============
*/
void VS_GetFilter(vsound_t *light, lightvolume_t *volume, vec3_t lmp,
                  vec3_t filter) {
  lFacet_t *facet;
  lsurfaceTest_t *test;
...
=============
*/
int VS_SplitLightVolume(lightvolume_t *volume, lightvolume_t *back,
  ...
  for (i = 0; i < volume->numplanes; i++) {
    ...
    if (f.numplanes >= MAX_POINTS_ON_FIXED_WINDING) {
      _printf("WARNING: VS_SplitLightVolume -> MAX_POINTS_ON_FIXED_WINDING "
              "overflowed\n");
...
=============
*/
void VS_PlaneForEdgeToWinding(vec3_t p1, vec3_t p2, winding_t *w,
...
=============
*/
void VS_R_FloodLight(vsound_t *light, lightvolume_t *volume, int cluster,
                     int firstportal);
...
void VS_R_CastLightAtSurface(vsound_t *light, lightvolume_t *volume) {
  ...
  VS_LightSurfaceWithVolume(volume->surfaceNum, volume->facetNum, light,
                            volume);
  //
...
int VS_R_SplitLightVolume(vsound_t *light, lightvolume_t *volume,
...
=============
*/
void VS_R_SubdivideDirectedAreaLight(vsound_t *light, int nodenum,
                                     winding_t *w) {
  int leafnum, res;
  dnode_t *node;
  dplane_t *plane;
...
void VS_FloodLight(vsound_t *light) {
  ...
  case LIGHT_POINTRADIAL: {
    ...
    for (i = 0; i < 3; i++) {
      ...
      for (j = -1; j <= 1; j += 2) {
        ...
        for (k = 0; k < volume.numplanes; k++) {
          VS_PlaneFromPoints(&volume.planes[k], light->origin,
  ...
  case LIGHT_POINTSPOT: {
    ...
    for (i = 0; i < volume.numplanes; i++) {
      VS_PlaneFromPoints(&volume.planes[i], light->origin,
    ...
    for (i = -1; i <= 1; i += 2) {
      for (j = -1; j <= 1; j += 2) {
        for (n = 0; n < 2; n++) {
          ...
          VS_PlaneFromPoints(&volume.endplane, volume.points[0],
...
=============
*/
void VS_FloodLightThread(int num) { VS_FloodLight(vsounds[num]); }
...
void VS_SurfaceRadiosity(int num) {
  ...
  for (x = 0; x < ds->lightmapWidth; x++) {
    for (y = 0; y < ds->lightmapHeight; y++) {
      ...
      // ok this is crap
      vsound.photons = VectorLength(color) * 0.05 * lightPointScale /
