...
md3Header_t *R_LoadMD3(const char *mod_name) {
  ...
  for (i = 0; i < md3->numSurfaces; i++) {
    ...
    if (surf->numVerts > SHADER_MAX_VERTEXES) {
      Error("R_LoadMD3: %s has more than %i verts on a surface (%i)", mod_name,
...
============
*/
void InsertMD3Model(const char *modelName, vec3_t origin, float angle,
                    tree_t *tree) {
  int i, j;
  md3Header_t *md3;
  md3Surface_t *surf;
  ...
  for (i = 0; i < md3->numSurfaces; i++) {
    ...
    for (j = 0; j < surf->numVerts; j++, st++, xyz++) {
      ...
      outv->xyz[0] = origin[0] + MD3_XYZ_SCALE * (xyz->xyz[0] * angleCos -
                                                  xyz->xyz[1] * angleSin);
      outv->xyz[1] = origin[1] + MD3_XYZ_SCALE * (xyz->xyz[0] * angleSin +
                                                  xyz->xyz[1] * angleCos);
...
============
*/
void InsertASEModel(const char *modelName, vec3_t origin, float angle,
                    tree_t *tree) {
  int i, j;
  drawVert_t *outv;
  mapDrawSurface_t *out;
