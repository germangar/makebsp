...
qboolean PlaneEqual(plane_t *p, vec3_t normal, vec_t dist) {
  ...
  if (fabs(p->normal[0] - normal[0]) < NORMAL_EPSILON &&
    ...
    return qtrue;
#else
  if (p->normal[0] == normal[0] && p->normal[1] == normal[1] &&
      p->normal[2] == normal[2] && p->dist == dist)
    return qtrue;
#endif
  return qfalse;
}
...
void SetBrushContents(bspbrush_t *b) {
  ...
  if ((contents & CONTENTS_DETAIL) && (contents & CONTENTS_STRUCTURAL)) {
    _printf(
        "Entity %i, Brush %i: mixed CONTENTS_DETAIL and CONTENTS_STRUCTURAL\n",
...
void AddBrushBevels(void) {
  ...
  for (axis = 0; axis < 3; axis++) {
    for (dir = -1; dir <= 1; dir += 2, order++) {
      ...
      if (i == buildBrush->numsides) { // add a new side
        ...
        for (axis = 0; axis < 3; axis++) {
          for (dir = -1; dir <= 1; dir += 2) {
            ...
            for (k = 0; k < buildBrush->numsides; k++) {
              ...
              if (PlaneEqual(&mapplanes[buildBrush->sides[k].planenum], normal,
                             dist))
                break;
...
NOTE : it would be "cleaner" to have seperate functions to parse between old and
new brushes
=================
*/
void ParseRawBrush() {
  ...
  do {
    ...
    if (!strcmp(token, "}"))
      break;
    // Timo : brush primitive : here we may have to jump over brush epairs (
    // only used in editor )
    if (g_bBrushPrimit == BPRIMIT_NEWBRUSHES) {
      do {
        if (strcmp(token, "("))
...
ParseBrush
  ...
  qboolean parameter to true -> parse new brush primitive format ( else use old
format )
=================
*/
...
qboolean ParseMapEntity(void) {
  ...
  if (strcmp(token, "{")) {
    Error("ParseEntity: { not found, found %s on line %d - last entity was at: "
          "<%4.2f, %4.2f, %4.2f>...",
          token, scriptline, entities[num_entities].origin[0],
          entities[num_entities].origin[1], entities[num_entities].origin[2]);
  }
