...
====================
*/
void SplitMeshByPlane(mesh_t *in, vec3_t normal, float dist, mesh_t **front,
                      mesh_t **back)
{
    int w, h, split;
    ...
    for (w = 0; w < in->width; w++)
    {
        for (h = 0; h < in->height; h++)
        {
            ...
            {
                b->verts[h * b->width + w - split + backAprox] =
    ...
    for (h = 0; h < in->height; h++)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            dv->normal[i] = v1->normal[i] + frac * (v2->normal[i] - v1->normal[i]);
        }
        for (i = 0; i < 2; i++)
        {
...
qboolean ChopFaceByBrush(mapDrawSurface_t *ds, bspbrush_t *b)
{
    ...
    for (i = 0; i < numOutside; i++)
    {
        DrawSurfaceForSide(ds->mapBrush, s, outside[i]);
    ...
    for (b = entities[0].brushes; b; b = b->next)
    {
        ...
        for (i = 0; i < b->numsides; i++)
        {
            if (b->sides[i].shaderInfo &&
                (b->sides[i].shaderInfo->contents & CONTENTS_FOG))
            {
