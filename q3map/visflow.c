...
/*
  ...
  for p2 = all other v_portals in leaf
	get sperating planes
	for all v_portals that might be seen by p2
...
Source, pass, and target are an ordering of v_portals.
...
Flood fill through the v_leafs
...
//	CheckStack (leaf, thread);
	...
	// check all v_portals for flowing into other v_leafs	
		...
		/* MrE: portal trace debug code
		{
			...
			if (j >= sizeof(portaltrace)/sizeof(int) - 1)
			{
				if (p->num == portaltrace[j])
					n = 0; //traced through all the v_portals
			}
		}
		*/
...
void PortalFlow (int portalnum)
{
	...
	c_might = CountBits (p->portalflood, v_numportals*2);
	...
	c_can = CountBits (p->portalvis, v_numportals*2);

	qprintf ("portal:%4i  mightsee:%4i  cansee:%4i (%i chains)\n", 
		(int)(p - v_portals),	c_might, c_can, data.c_chains);
}
...
void RecursivePassageFlow (vportal_t *portal, threaddata_t *thread, pstack_t *prevstack)
{
	...
	nextpassage = passage;
	// check all v_portals for flowing into other v_leafs	
	for (i = 0; i < leaf->numportals_in_leaf; i++, passage = nextpassage)
	{
		p = leaf->portals_in_leaf[i];
		if ( p->removed ) {
			continue;
...
MrE: create passages from one portal to all the v_portals in the leaf the portal leads to
	 every passage has a cansee bit string with all the v_portals that can be
...
all seperating planes, and both v_portals must be behind the new portal
...
void BasePortalVis (int portalnum)
{
	...
	for (j=0, tp = v_portals ; j<v_numportals*2 ; j++, tp++)
	{
		if (j == portalnum)
			continue;
		if (tp->removed)
	...
	p->nummightsee = CountBits (p->portalflood, v_numportals*2);
...
void RecursiveLeafBitFlow (int leafnum, byte *mightsee, byte *cansee)
{
	...
	for (i=0 ; i<leaf->numportals_in_leaf ; i++)
	{
		...
		// if this portal can see some v_portals we mightsee, recurse
		more = 0;
		for (j=0 ; j<portallongs ; j++)
		{
...
void BetterPortalVis (int portalnum)
{
	...
	p->nummightsee = CountBits (p->portalvis, v_numportals*2);
	c_vis += p->nummightsee;
}
