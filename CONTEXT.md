...
- **Dynamic Mesh Buffer Sizing**: Mesh subdivision routines (`SubdivideMeshQuads`, `SubdivideMesh`, `RemoveLinearMeshColumnsRows`, and their 32-bit counterparts) dynamically allocate work buffers matching the exact input/max bounds. This completely eliminates legacy fixed 1024-axis (`MAX_EXPANDED_AXIS`) limits, supporting arbitrary high-resolution lightmap atlases (2048, 4096, etc.).
- **Decoupled Atlas Resolution & External Storage**: Setting or adapting lightmap image sizes (`-lightmapimagesize`, `_lightmapImageSize`) is strictly decoupled from `externalLightmaps`. Lightmaps remain stored internally within the `.bsp` lump by default (e.g. for QFusion/Warfork), and are only exported as external image files when explicitly configured in the game profile (`xonotic.json`) or via the `-externallightmaps` CLI switch.
...
## 18. Decals & Text Projection Pipeline (Geometry, Fonts, & Adaptive Extrusion)
The toolchain implements a robust, unified projection pipeline for decals and dynamic text, converting them from declarative frustums or font instructions into perfectly conformed, lightmapped `MST_TRIANGLE_SOUP` geometry.

- **18.1 Decal Entity Hierarchy**:
  - `_decal`: Geometry-based projection. Evaluates structural brush faces or patch meshes acting as projection sources, automatically deriving normals (`CalculateDecalFallbackNormal`) or using `target_position`.
  - `misc_decal`: Point entity parameterized by rotation, distance, dimensions, and orientation, generating a quad projection source.
  - `misc_text_decal`: Dynamic text entity. A two-pass engine first measures total string width (accounting for tracking and glyph advance) for exact alignment (left, center, right, top, middle, baseline), then loops per-character to generate precise bounding frustums mapped to individual font atlas UVs.
- **18.2 TrueType Font Baking & Runtime Parsing**:
  - **Atlas Baker**: The built-in `-fontatlas` CLI routine (powered by `stb_truetype` and `stb_rect_pack`) performs a 12-iteration binary bisection search to fit all 95 printable ASCII glyphs into a maximally sized power-of-two texture. It exports a 32-bit RGBA TGA and a JSON descriptor containing ascent, descent, and precise per-glyph metrics (UVs, offsets, advance).
  - **Parser**: `LoadFontDescriptor` caches JSON font definitions, dynamically generating fallback characters (caret, question mark) for missing ASCII requests.
- **18.3 6-Plane Frustum & Barycentric Texture Matrix**:
  - Every decal projector is converted into a bounding frustum composed of 6 planes: a front plane, back plane (clamped to `distance`), and four boundary side planes pointing outward.
  - A mathematically continuous texture projection matrix (`texMat[2][4]`) is solved using barycentric coordinate derivatives from 3 non-collinear corners of the original projection quad. This ensures unified UV mapping with no per-triangle discontinuities across highly subdivided or irregular receiving geometry.
- **18.4 Curvature-Adaptive Extrusion & MeshLib Decimation**:
  - **Curved Surface Tessellation**: Patch receivers are finely subdivided along engine-matching diagonal strips, face-culled, and clipped strictly (`ClipWindingEpsilonStrict`) against the 6-plane frustum.
  - **Curvature Extrusion**: To prevent Z-fighting on curved walls, the geometry applies a non-linear outward normal extrusion. It calculates edge curvature $k$, diffuses it with Laplacian smoothing, and dynamically biases the extrusion distance: `dist = MIN_EXTRUSION + sqrt(k) * CONCAVITY_SCALE`, keeping flat decals flush while sharply pushing out concave joints.
  - **Decimation**: The finely tessellated and extruded geometry is piped into `MRMesh` decimation (`mrMeshDecimateWithAttributes`). Internal coplanar faces are decimated, collapsing thousands of sub-triangles while rigorously preserving silhouette boundaries and vertex UVs/colors.
- **18.5 BSP Model Surface Emission**:
  - Decal meshes are emitted as `MST_TRIANGLE_SOUP` surfaces (`isDecal = qtrue`, `miscModel = qtrue`), inherently merging into the `xatlas` lightmapping pipeline and inheriting vertex color/alpha, `samplesize`, and smooth shading properties just like any standard 3D model.
